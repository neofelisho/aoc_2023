...
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green`
	...
	puzzleInput = `Game 1: 13 red, 18 green; 5 green, 3 red, 5 blue; 5 green, 9 red, 6 blue; 3 blue, 3 green
...
func main() {
	games := parseGame(puzzleInput)
	partOne(games)
	partTwo(games)
}

func partTwo(games map[int][]Set) {
	sum := 0
	for _, sets := range games {
		minSet := minimumSet(sets)
		sum += minSet.Red * minSet.Green * minSet.Blue
	}
	println("The sum of power =", sum)
}

func minimumSet(sets []Set) Set {
	minSet := Set{
		Red:   sets[0].Red,
		Green: sets[0].Green,
		Blue:  sets[0].Blue,
	...
	for i := 1; i < len(sets); i++ {
		if sets[i].Red > minSet.Red {
			minSet.Red = sets[i].Red
		}
		if sets[i].Green > minSet.Green {
			minSet.Green = sets[i].Green
		}
		if sets[i].Blue > minSet.Blue {
			minSet.Blue = sets[i].Blue
		}
	}
	return minSet
}

func partOne(games map[int][]Set) {
	sum := 0
	...
	println("The sum of IDs =", sum)
}

func allValid(sets []Set) bool {
...
func parseGame(s string) map[int][]Set {
	gamePattern := regexp.MustCompile(`Game (\d+): (.+)`)
	matches := gamePattern.FindAllStringSubmatch(s, -1)
	games := make(map[int][]Set)
	for _, match := range matches {
		gameId, _ := strconv.Atoi(match[1])
		setStrings := strings.Split(match[2], "; ")
...
func parseSets(setStrings []string) []Set {
	setPattern := regexp.MustCompile(`(\d+) (\w+)`)
	sets := make([]Set, 0)
	for _, setString := range setStrings {
		matches := setPattern.FindAllStringSubmatch(setString, -1)
		set := Set{}
		for _, match := range matches {
