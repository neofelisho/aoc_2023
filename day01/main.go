...
7pqrstsixteen`
	...
	calibrationDocument = `9sixsevenz3
...
func calibrationValue(document string) int {
	...
	for scanner.Scan() {
		...
		//value += findDigits(text)
		value += findRealDigits(text)
	}

	if err := scanner.Err(); err != nil {
		fmt.Println("Error occurred while reading the calibration document:", err)
	}
...
func findDigits(s string) int {
	...
	value := first*10 + last
	fmt.Println("Line value:", value)
	return value
}

// Part Two
func findRealDigits(input string) int {
	pattern := `one|two|three|four|five|six|seven|eight|nine`
	regExp := regexp.MustCompile(pattern + `|\d`)
	firstMatch := regExp.FindString(input)

	reversedInput := reverseString(input)
	reversedPattern := reverseString(pattern)
	regExp = regexp.MustCompile(reversedPattern + `|\d`)
	lastMatch := regExp.FindString(reversedInput)

	first := convertToDigit(firstMatch)
	last := convertToDigit(reverseString(lastMatch))
	value := first*10 + last
	fmt.Println("Line value:", value)
	return value
}

func convertToDigit(match string) int {
	switch match {
	case "one":
		return 1
	case "two":
